...
module ReactiveHttp =
    ...
    let CreateWebServerObservable url =
        ...
        listener.Start()
        let obs = Observable.Defer(fun () -> listener.GetContextAsync().ToObservable()).Repeat()
        listener,obs

    let StartWebStatusApi url (observable:IObservable<ConnectedClient list>) =
        let listener, obs = CreateWebServerObservable url
        let mostrecent = observable.Sample(TimeSpan.FromSeconds(1.0)).MostRecent([])
        obs.Zip(mostrecent, fun a b -> printfn "Received request"
                                       let res = sprintf "%i" b.Length
