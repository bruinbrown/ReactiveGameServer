...
module Core = 
    ...
    let StartListening(port : int) = 
        ...
        let o = 
            ...
            |> Observable.map 
                   (fun t -> 
                   t.RemoteEndPoint, (Deserialize<ClientMessage> t.Buffer))
        client, o
    ...
    let HandleConnectionRequest (client : UdpClient) 
        ...
        printfn 
            "Received a connection request from user %s at IP %s, assigning ID %s" 
            un (connection.ToString()) (id.ToString())
        let currState = 
            worldState |> List.map (fun t -> 
                              { Id = t.Id
                                Position = t.Position
                                Username = t.Username } : PlayerInfo)
        
        let clientMsg = ConnectionConfirmation(id, pos, currState) |> Serialize
        client.SendAsync(clientMsg, clientMsg.Length, connection)
        |> Async.AwaitTask
        |> Async.RunSynchronously
        |> ignore
        let msg = ClientJoin(un, id, pos) |> Serialize
        worldState |> List.iter (fun t -> 
                          client.SendAsync(msg, msg.Length, t.Connection)
                          |> Async.AwaitTask
                          |> Async.RunSynchronously
                          |> ignore)
    ...
    let HandleDisconnectionRequest (client : UdpClient) worldState connection = 
        let disconnecting, others = 
            worldState |> List.partition (fun t -> 
                              if t.Connection = connection then true
                              else false)
        printfn "Player at %s disconnecting" (connection.ToString())
        let msg = ClientLeft(disconnecting.Head.Id) |> Serialize
        others |> List.iter (fun t -> 
                      client.SendAsync(msg, msg.Length)
                      |> Async.AwaitTask
                      |> Async.RunSynchronously
                      |> ignore)
        others
    ...
    let HandleCommand (client : UdpClient) worldState connection cmd = 
        let current, others = 
            worldState |> List.partition (fun t -> t.Connection = connection)
        ...
        | Movement(d) -> 
            let dx, dy = 
                ...
                | Down -> (0, 1)
                | Right -> (1, 0)
            printfn "Received movement command from %s, Moving %A" 
                (connection.ToString()) (dx, dy)
