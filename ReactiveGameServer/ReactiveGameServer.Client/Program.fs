...
module Core =
    ...
    let PerformHandshake (localPort:int) endPoint username =
        ...
        try
            ...
            match msg with
            | ConnectionConfirmation(id, pos,state) -> printfn "Successfully connected to the server, assigned id %s" (id.ToString())
                                                       printfn "Assigned spawn at position (%i, %i)" (fst pos) (snd pos)
    ...
    let RemovePlayer (form:Form) (id:Guid) =
        let Remove () =
            form.Controls.RemoveByKey(id.ToString())
        if form.InvokeRequired then
    ...
    let MovePlayer (client:UdpClient) (form:Form) state (id:Guid) direction =
        ...
        let Move () =
            control.Location <- Point(loc.X + dx, loc.Y + dy)
        if form.InvokeRequired then
            let cb = FormsDelegate(Move)
            form.Invoke(cb) |> ignore
        Move ()
        let user, other = state |> List.partition (fun t -> match t with
                                                            | Player.Local _ -> true
                                                            | _ -> false)
        let user = user.Head
        let user = match user with
                   | Player.Local(client, history) -> let px,py = client.Position
                                                      Player.Local({ client with Position = (px + dx, py + dy) }, (dx,dy)::history)
                   | _ -> failwith "A fatal error has occurred, we're trying to move a remote player and that's just not possible"
        user :: other

    let HandleClientJoin (form:Form) worldState (id:Guid) username (x,y) =
        let client = ({ Id = id; Position = (x,y) }:Client) |> Player.Remote
        AddPlayer form id username (x,y)
        client :: worldState

    let HandleClientLeave (form:Form) worldState (id:Guid) =
        let client = worldState
                     |> List.filter (fun t-> match t with
                                             | Player.Remote p -> if p.Id = id then false else true
                                             | _ -> true)
    ...
    let HandleWorldUpdate (form:Form) worldState (update:Client list) =
        let local = worldState
                    |> List.sortBy (fun t -> match t with
        ...
        List.map2 (fun local (remote:Client) -> match local with
                                                | Player.Local (client, history) -> let newPos = ApplyHistory remote.Position history
                                                                                    MoveRemote form client.Id newPos
                                                                                    Player.Local({ client with Position = newPos }, [])
                                                | Player.Remote (client) -> MoveRemote form client.Id remote.Position
                                                                            Player.Remote({ client with Position = remote.Position })) local remote
    ...
    let RunGame (client:UdpClient) state (id:Guid) username (x,y) =
        ...
        AddPlayer form id username (x,y)
        state
        |> List.iter (fun t -> AddPlayer form t.Id t.Username t.Position)

        let others = state
                     |> List.map (fun t -> Player.Remote({ Id = t.Id; Position = t.Position}))
        let keyObs = form.KeyPress
                          |> Observable.map (fun t -> match t.KeyChar with
                                                      | 'w' -> Up
                                                      | 's' -> Down
                                                      | 'a' -> Left
                                                      | 'd' -> Right
        ...
        let remoteObs = UdpClientToObservable client
                        |> Observable.map (fun t -> Remote(t))

        let merged = Observable.merge keyObs remoteObs
                     |> Observable.scan 
                          (fun state t -> match t with
                                          | Local(t) -> MovePlayer client form state id t
                                          | Remote(t) -> let msg = Deserialize<ServerMessage> t.Buffer
                                                         HandleMessage form state msg) (Player.Local({ Id = id; Position = (x,y) }, []) :: others)
    ...
    let main argv = 
        ...
        match status with
        | Success (client, id, pos, state) -> RunGame client state id username pos
